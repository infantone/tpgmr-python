...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "in un file YAML (stesso formato column-major usato da Franka/libfranka)."
        )
    )
...
def _franka_state_to_matrix(state: FrankaState) -> np.ndarray:
    ...
    ATTENZIONE: Franka usa column-major:
    [r00, r10, r20, 0,
     r01, r11, r21, 0,
     r02, r12, r22, 0,
     px,  py,  pz, 1]
    ...
    # order='F' = column-major → recupera la matrice corretta senza cambiare convenzione
...
def _write_transform(destination: Path, matrix: np.ndarray) -> None:
    """
    Scrive O_T_EE in YAML mantenendo il formato column-major di Franka.
    """
...
def main() -> int:
    ...
    try:
        ...
        vec = matrix.reshape(16, order="F")
        print(f"Posa salvata in {destination}")
        print(f"O_T_EE (column-major): {vec.tolist()}")

        # Posizione corretta: prima colonna = rot, ultima colonna = traslazione
        position = matrix[:3, 3]
